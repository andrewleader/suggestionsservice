...
namespace SuggestionsService.Helpers
{
    ...
    {
        public static async Task<Event[]> GetCalendarEventsAsync(GraphServiceClient graphClient, HttpContext httpContext)
        {
            try
            {
                ICalendarEventsCollectionPage eventsPage = await graphClient.Me.Calendar.Events.Request().GetAsync();
                return eventsPage.CurrentPage.ToArray();
            }
