...
using SuggestionsService.Controllers.Api;
using AdaptiveCards.Rendering.Html;
using AdaptiveCards;
using SuggestionsService.Model;

namespace SuggestionsService.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index(string email)
        {
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                try
                {
                    var suggestions = await SuggestionsController.GetSuggestionsAsync(graphClient, HttpContext);
                    model.Suggestions = suggestions;
                    ViewData["Suggestions"] = JsonConvert.SerializeObject(suggestions);
                }
                catch { }
            }
